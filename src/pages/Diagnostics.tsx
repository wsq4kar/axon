...
export default function Diagnostics() {
  ...
  const run = async () => {
    ...
    await check('reach', async () => { if (!node) return [false, 'нечего проверять']; const ms = await backend.tcpPing(node.server, node.port); return ms != null ? [true, `${node.server}:${node.port} — ${ms} мс`] : [false, `${node.server}:${node.port} не отвечает`, 'Попробуйте сервер с другим протоколом (Hysteria2, XHTTP) или включите фрагментацию TLS в настройках. Если не отвечает ни один — напишите в поддержку своего сервиса.']; });
